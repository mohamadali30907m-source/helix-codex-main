  ...
  if (type === "wink") {
    return (
      <mesh position={position} rotation={[0, 0, -0.12]}>
        ...
        <GlowMaterial color={COLORS.cyan} intensity={2.5} />
      </mesh>
    );
  }
...
function StatusLight() {
  return (
    <mesh position={[0.52, 0.82, 0.665]}>
      ...
      <GlowMaterial color={COLORS.green} intensity={2.8} />
    </mesh>
  );
}
...
function ShoulderJoint({ side }) {
  const direction = side === "left" ? -1 : 1;

  return (
    <group position={[direction * 1.2, 0.45, 0]}>
...
function Arm({ side, servoAngle }) {
  ...
  useFrame(() => {
    ...
    armRef.current.rotation.z = radians * direction * 0.45;
  });
...
function ServoModule({ angle }) {
  const servoRef = useRef();

  useFrame(() => {
    if (!servoRef.current) return;

    servoRef.current.rotation.z = (angle * Math.PI) / 180;
  });
...
export default function VirtualRobot({ servoAngle = 0 }) {
  return (
    <group position={[0, 0, 0]}>
      ...
      {/* Shoulder joints */}
      <ShoulderJoint side="left" />
      <ShoulderJoint side="right" />
